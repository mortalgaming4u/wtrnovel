...
def get_full_toc_url(book_url):
    ...
    r = requests.get(book_url, headers=BASE_HEADERS, timeout=15)
    r.encoding = 'utf-8'
    soup = BeautifulSoup(r.text, 'html.parser')
    toc_link = soup.find('a', string=lambda t: t and '完整章節' in t)
    if not toc_link:
        raise RuntimeError("Full TOC link not found.")
...
def parse_toc(toc_url):
    ...
    while toc_url:
        r = requests.get(toc_url, headers=BASE_HEADERS, timeout=15)
        r.encoding = 'utf-8'
        soup = BeautifulSoup(r.text, 'html.parser')
        ...
        # Look for pagination link for TOC
...
def fetch_and_clean(ch_url):
    ...
    try:
        r = requests.get(ch_url, headers=BASE_HEADERS, timeout=15)
        r.encoding = 'utf-8'
        soup = BeautifulSoup(r.text, 'html.parser')
        ...
        print(f"[ERROR] {e} while fetching {ch_url}")
        return ''
